...
(defn external-query
  ...
  {:nested {:path "hits",
            :inner_hits {},
            :query {:bool {:must   (when (not-blank? keyword) {:match {(->lng-keyword "hits.terms.%s" lng) {:query (lower-case keyword) :operator "and" :fuzziness "AUTO:8,12"}}})
...
(defn- koulutustyyppi-filters
  [field]
  (->filters-aggregation field '["amm" "amm-tutkinnon-osa" "amm-osaamisala" "lk" "amk" "yo" "amk-alempi" "amk-ylempi" "kandi" "kandi-ja-maisteri" "maisteri" "tohtori", "tuva"]))
